...
import { normalizeMindMapLayouts, normalizeMindMapLayoutState } from '../../mindmap/mindMapLayoutState'
import {
  ...
  return {
    ...
    loadDoc: async (path) => {
      try {
        ...
        doc.mindMapLayouts = normalizeMindMapLayouts(doc.mindMapLayouts, doc.mindMapLayout)
        const collapsedIds = new Set<string>()
        collectCollapsedIds(doc.root, collapsedIds)
    ...
    saveDoc: async (customPath = null) => {
      ...
      if (!path) {
        const created = await api.createServerDocument(
          state.currentDoc.title || '未命名文档',
          null,
          state.currentDoc,
        )
    ...
    importDoc: async (path, format) => {
      try {
        ...
        doc = {
          ...
          mindMapLayouts: normalizeMindMapLayouts(doc.mindMapLayouts, doc.mindMapLayout),
        }
    ...
    applyImportPreview: (preview, options) => {
      const importedDoc = {
        ...
        mindMapLayouts: normalizeMindMapLayouts(preview.document.mindMapLayouts, preview.document.mindMapLayout),
      }
