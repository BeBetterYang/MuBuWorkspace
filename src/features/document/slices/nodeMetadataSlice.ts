...
export function createNodeMetadataSlice(context: DocumentStoreContext): NodeMetadataActions {
  ...
  return {
    updateNodeFormatting: (nodeId, format) => {
      mutateNodeProperty(nodeId, (node, now) => {
        const nextFormat = { ...node.format, ...format }
        for (const key of Object.keys(nextFormat) as Array<keyof typeof nextFormat>) {
          if (nextFormat[key] === undefined || nextFormat[key] === '') delete nextFormat[key]
        }
        if (JSON.stringify(node.format ?? {}) === JSON.stringify(nextFormat)) return node
        return { ...node, format: Object.keys(nextFormat).length ? nextFormat : undefined, updatedAt: now }
      })
    },
    ...
    updateNodeSummary: (nodeId, summary) => {
      mutateNodeProperty(nodeId, (node, now) => {
        const normalized = summary?.text.trim()
          ? {
            ...summary,
            text: summary.text.trim(),
            nodeIds: Array.from(new Set(summary.nodeIds)),
            note: normalizeNote(summary.note ?? ''),
            format: summary.format && Object.keys(summary.format).length ? summary.format : undefined,
          }
          : undefined
        if (JSON.stringify(node.summary) === JSON.stringify(normalized)) return node
        return { ...node, summary: normalized, updatedAt: now }
      })
    },
