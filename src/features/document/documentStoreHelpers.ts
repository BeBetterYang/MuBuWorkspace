...
import { pruneMindMapLayouts, pruneMindMapLayoutState } from '../mindmap/mindMapLayoutState'
...
export function getDocumentWithVersionForSave(doc: OutlineDocument): OutlineDocument {
  ...
  const mindMapLayouts = pruneMindMapLayouts(doc.mindMapLayouts, doc.root, mindMapLayout)
  if (!mindMapLayout && !mindMapLayouts) return doc
  return {
    ...doc,
    mindMapLayout,
    mindMapLayouts,
