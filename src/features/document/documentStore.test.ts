...
describe('documentStore', () => {
  ...
  it('creates a server document before saving an unsaved document', async () => {
    useDocumentStore.setState({
      currentDoc: createDocument(),
      currentFilePath: null,
      isDirty: true,
    })
    apiMock.createServerDocument.mockResolvedValueOnce({
      item: { id: 'server-doc', type: 'document', name: 'test', parentId: null, createdAt: 1, updatedAt: 1 },
    ...
    expect(saved).toBe(true)
    expect(apiMock.saveDocument).toHaveBeenCalledWith('server-doc', expect.any(Object))
    expect(useDocumentStore.getState().currentFilePath).toBe('server-doc')
    expect(useDocumentStore.getState().isDirty).toBe(false)
  })
  ...
  it('keeps an independent saved layout for every strategy in the document', async () => {
    await loadFixtureDoc()
    ...
    useDocumentStore.getState().commitMindMapLayout({
      ...
      nodes: {
        'node-1': { position: { x: 420, y: 280 }, source: 'manual', locked: true },
      },
    })

    const layouts = useDocumentStore.getState().currentDoc?.mindMapLayouts
    expect(layouts?.['classic-dagre'].nodes['node-1'].position).toEqual({ x: 120, y: 80 })
    expect(layouts?.['free-canvas'].nodes['node-1'].position).toEqual({ x: 420, y: 280 })
