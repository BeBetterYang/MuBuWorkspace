...
export const MindMapExportMenu: React.FC<MindMapExportMenuProps> = ({ status, onExport, className }) => {
  return (
    <div className={className ?? 'ui-popover absolute left-4 top-16 z-20 w-44 p-1.5 text-xs'}>
      <button
        type="button"
        role="menuitem"
        disabled={status === 'exporting'}
        onClick={() => onExport('png')}
        className="flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-[var(--color-charcoal)] enabled:hover:bg-[var(--color-tint-lavender)] disabled:text-zinc-300"
      >
