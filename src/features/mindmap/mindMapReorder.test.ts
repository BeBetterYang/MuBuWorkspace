...
describe('mindMapReorder', () => {
  ...
  it('magnetically resolves the nearest node just outside its bounds', () => {
    expect(resolveMindMapDragTarget(
      { id: 'dragged', position: { x: 360, y: 18 }, width: 80, height: 44 },
      [
        { id: 'target', position: { x: 200, y: 0 }, width: 150, height: 80 },
        { id: 'far-away', position: { x: 900, y: 0 }, width: 150, height: 80 },
      ],
    )).toEqual({ targetNodeId: 'target', zone: 'child' })
  })
