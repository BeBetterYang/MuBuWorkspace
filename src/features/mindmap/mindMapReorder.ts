...
export const MIND_MAP_SNAP_DISTANCE = 56
...
export function resolveMindMapDragTarget(
  ...
  let closest: { target: ResolvedDragTarget; distance: number } | null = null
  ...
  for (const node of nodes) {
    ...
    const distanceX = Math.max(left - center.x, 0, center.x - right)
    const distanceY = Math.max(top - center.y, 0, center.y - bottom)
    const distance = Math.hypot(distanceX, distanceY)
    if (distance > MIND_MAP_SNAP_DISTANCE || closest && distance >= closest.distance) continue

    closest = {
      target: {
        targetNodeId: node.id,
        zone: distanceY > 0 ? (center.y < top ? 'before' : 'after') : getMindMapDropZone(center.y, top, height),
      },
      distance,
    }
  }
