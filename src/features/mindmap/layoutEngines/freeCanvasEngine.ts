...
export const freeCanvasEngine: MindMapLayoutEngine = {
  layout(input) {
    ...
    const persisted = input.persistedLayout?.strategy === 'free-canvas'
      ? normalizeMindMapLayoutState(input.persistedLayout, 'free-canvas')
      : undefined
    ...
    return createResult(
      input,
      nodes,
      attachDirectionalEdgeHandles(input.graphData.edges, nodes, input.nodeSizes, true),
      { sourceByNodeId },
    )
  },
}
