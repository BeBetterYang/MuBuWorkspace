...
export function normalizeMindMapLayouts(
  layouts: Record<string, MindMapLayoutState> | undefined,
  activeLayout?: MindMapLayoutState,
): Record<string, MindMapLayoutState> | undefined {
  const normalized = Object.entries(layouts ?? {}).reduce<Record<string, MindMapLayoutState>>((result, [strategy, layout]) => {
    const value = normalizeMindMapLayoutState(layout, strategy)
    if (value) result[value.strategy] = value
    return result
  }, {})
  const active = normalizeMindMapLayoutState(activeLayout)
  if (active) normalized[active.strategy] = active
  return Object.keys(normalized).length > 0 ? normalized : undefined
}

export function pruneMindMapLayouts(
  layouts: Record<string, MindMapLayoutState> | undefined,
  root: LayoutTreeNode,
  activeLayout?: MindMapLayoutState,
): Record<string, MindMapLayoutState> | undefined {
  const normalized = normalizeMindMapLayouts(layouts, activeLayout)
  if (!normalized) return undefined
  return Object.fromEntries(Object.entries(normalized).flatMap(([strategy, layout]) => {
    const pruned = pruneMindMapLayoutState(layout, root)
    return pruned ? [[strategy, pruned]] : []
  }))
}

export function getMindMapLayoutForStrategy(
  document: { mindMapLayout?: MindMapLayoutState; mindMapLayouts?: Record<string, MindMapLayoutState> } | null | undefined,
  strategy: MindMapLayoutStrategy,
): MindMapLayoutState | undefined {
  const saved = document?.mindMapLayouts?.[strategy]
  if (saved) return normalizeMindMapLayoutState(saved, strategy)
  return document?.mindMapLayout?.strategy === strategy
    ? normalizeMindMapLayoutState(document.mindMapLayout, strategy)
    : undefined
}
