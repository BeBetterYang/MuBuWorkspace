...
vi.mock('../services/siweiApi', () => ({
  ...
  getServerWorkspace: vi.fn(() => Promise.resolve({ items: [] })),
  createServerFolder: vi.fn(),
  createServerDocument: vi.fn(),
  deleteServerItem: vi.fn(),
...
describe('App', () => {
  ...
  it('does not show task controls in the top toolbar', async () => {
    useDocumentStore.setState({ viewMode: 'outline' })

    render(<App />)

    await waitFor(() => expect(screen.getByTitle('我的文档 / 测试文档')).toBeInTheDocument())
    expect(screen.queryByText('当前范围内没有待办')).not.toBeInTheDocument()
  })
